      ...
      <div className="bg-white px-4 py-16 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-7xl">
          <div className="mb-10 max-w-3xl">
            ...
            <h1 className="text-4xl font-semibold tracking-tight text-gray-900 sm:text-5xl">A collaborative space for builders, learners, and leaders</h1>
            <p className="mt-4 text-lg text-gray-600">Our community brings together young professionals, students, mentors, and volunteers who want to solve local and global problems through technology.</p>
          </div>

          <div className="mb-12 grid gap-6 md:grid-cols-3">
            {stats.map((stat) => (
              <div key={stat.label} className="rounded-2xl border border-gray-200 bg-gray-50 p-6">
          ...
          <div className="grid gap-6 md:grid-cols-2 xl:grid-cols-3">
            {cards.map((card) => {
              ...
              return (
                <Link key={card.title} href={card.href} className="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm transition-all hover:-translate-y-1 hover:shadow-xl">
                  ...
                  <div className="mb-2 flex items-center gap-2">
                    {card.badge && <span className="rounded-full bg-[#0070f3]/10 px-2.5 py-1 text-xs font-medium text-[#0070f3]">{card.badge}</span>}
          ...
          <div className="mt-12 flex flex-wrap gap-4">
            <Link href="/join" className="inline-flex items-center gap-2 rounded-full bg-black px-6 py-3 text-sm font-medium text-white transition-colors hover:bg-gray-800">
              Become a member
              <ArrowRight className="h-4 w-4" />
            </Link>
            <Link href="/mentors" className="inline-flex items-center rounded-full border border-gray-300 px-6 py-3 text-sm font-medium text-gray-700 transition-colors hover:bg-gray-50">
              Explore mentors
            </Link>
          </div>
        </div>
