    ...
    >
      ...
      >
        ...
        <div className="relative">
          {/* Timeline line */}
          <div className="absolute left-4 top-0 bottom-0 w-px bg-gradient-to-b from-[#8CA0DE] via-[#4A7FC7] to-transparent" />
          ...
          <div className="space-y-8 pl-12">
            {events.map((event, index) => (
              ...
              >
                {/* Timeline dot */}
                <div className="absolute -left-10 top-0 w-5 h-5 rounded-full bg-[#1B2A56] border-2 border-[#8CA0DE] flex items-center justify-center">
                  <div className="w-2 h-2 rounded-full bg-[#8CA0DE]" />
                </div>

                <div className="group bg-white/5 border border-white/10 rounded-2xl p-6 hover:border-[#8CA0DE]/30 transition-all duration-300 hover:bg-white/10">
                  <div className="flex flex-wrap items-start justify-between gap-4">
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-2 flex-wrap">
                        <span className="text-xs font-medium px-2 py-1 rounded-full bg-[#1B2A56] text-[#8CA0DE] border border-[#1B2A56]">
                          {event.type}
                        </span>
                        {!event.isPast && (
                      ...
                      <h3 className="text-lg font-semibold text-white group-hover:text-[#8CA0DE] transition-colors">
                        {event.title}
                      </h3>
                      <div className="space-y-1 mt-2">
                        <div className="flex items-center gap-2 text-sm text-white/60">
                          <Calendar className="w-4 h-4 text-[#8CA0DE]" />
                          <span>{event.date}</span>
                        </div>
                        <div className="flex items-center gap-2 text-sm text-white/60">
                          <Clock className="w-4 h-4 text-[#8CA0DE]" />
                          <span>{event.time}</span>
                        </div>
                        <div className="flex items-center gap-2 text-sm text-white/60">
                          <MapPin className="w-4 h-4 text-[#8CA0DE]" />
                          <span>{event.location}</span>
                        </div>
                        <div className="flex items-center gap-2 text-sm text-white/60">
                          <Users className="w-4 h-4 text-[#8CA0DE]" />
                    ...
                    <button className="flex-shrink-0 px-4 py-2 rounded-full bg-[#8CA0DE] text-[#0A0B0E] font-medium hover:bg-white transition-colors text-sm flex items-center gap-1">
                      Register
